import { type DeployedMERC20API, MERC20API, type MERC20Providers } from 'midnight-erc20-port-api';
...
export interface DeployedBoardDeployment {
   ...
   */
  readonly api: DeployedMERC20API;
}
...
export interface DeployedBoardAPIProvider {
   ...
   * @param args The deployment arguments to use when deploying a new contract.
   ...
   */
  readonly resolve: (contractAddress?: ContractAddress, args?: deploymentArgs) => Observable<BoardDeployment>;
}
...
export class BrowserDeployedBoardManager implements DeployedBoardAPIProvider {
  ...
  #initializedProviders: Promise<MERC20Providers> | undefined;
  ...
  /** @inheritdoc */
  resolve(contractAddress?: ContractAddress, args?: deploymentArgs): Observable<BoardDeployment> {
    ...
    if (contractAddress) {
      ...
      if (!args) {
        throw new Error('No contract address or deployment arguments provided');
      }
      void this.deployDeployment(deployment, args);
    }
  ...
  public getProviders(): Promise<MERC20Providers> {
  ...
  private async deployDeployment(
    ...
    try {
      ...
      const api = await MERC20API.deploy(providers, args, this.logger);
  ...
  private async joinDeployment(
    ...
    try {
      const providers = await this.getProviders();
      const api = await MERC20API.join(providers, contractAddress, this.logger);
...
/** @internal */
const initializeProviders = async (logger: Logger): Promise<MERC20Providers> => {
